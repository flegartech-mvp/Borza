...
const chartCopy = {
  de: {
    ...
    visible: "sichtbare Kerzen",
    changed: "Preisänderung im sichtbaren simulierten Fenster",
    overlays: "Blau zeigt den 8-Perioden-Durchschnitt, Amber den VWAP.",
    powered: "Chart-Technik von",
  },
  sl: {
    watermark: "SIMULIRANO",
    stop: "STOP",
    target: "CILJ",
    support: "PODPORA",
    resistance: "ODPOR",
    visible: "vidnih sveč",
    changed: "sprememba cene v vidnem simuliranem oknu",
    overlays: "Modra prikazuje 8-obdobno povprečje, jantarna VWAP.",
    powered: "Tehnologija grafa",
  },
  en: {
    watermark: "SIMULATED",
    stop: "STOP",
    ...
    support: "SUPPORT",
    resistance: "RESISTANCE",
    visible: "visible candles",
    changed: "price change across the visible simulated window",
    overlays: "Blue is the 8-period moving average; amber is VWAP.",
    powered: "Chart technology by",
  },
};
...
export function MarketChart({
  ...
  useEffect(() => {
    ...
    const chart = createChart(container, {
      ...
      layout: {
        background: {
          ...
          color: styles.getPropertyValue("--surface-1").trim(),
        },
      ...
      timeScale: {
        ...
        borderColor: styles.getPropertyValue("--border-subtle").trim(),
      },
      rightPriceScale: {
        borderColor: styles.getPropertyValue("--border-subtle").trim(),
      },
    ...
    const maSeries = chart.addSeries(
      LineSeries,
      {
        color: styles.getPropertyValue("--electric").trim(),
        lineWidth: 2,
    ...
    const vwapSeries = chart.addSeries(
      LineSeries,
      {
        color: styles.getPropertyValue("--warning").trim(),
        lineWidth: 2,
        lineStyle: 2,
        priceLineVisible: false,
        lastValueVisible: false,
    ...
    createTextWatermark(chart.panes()[0], {
      ...
      vertAlign: "center",
      lines: [
        { text: copy.watermark, color: "rgba(128,145,160,0.14)", fontSize: 28 },
      ],
    });
  ...
  useEffect(() => {
    ...
    markerRef.current?.setMarkers(
      markers.map((marker) => ({
        ...
        position: marker.side === "entry" ? "belowBar" : "aboveBar",
        shape: marker.side === "entry" ? "arrowUp" : "arrowDown",
        color: marker.side === "entry" ? "#52cea2" : "#ed7272",
        text: marker.label,
      })),
    );
    ...
    if (stop !== undefined) {
      ...
      linesRef.current.push({
        remove: () => candleSeries.removePriceLine(line),
      });
    }
    if (target !== undefined) {
      const line = candleSeries.createPriceLine({
  ...
  return (
    ...
    >
      ...
      <figcaption className="border-t border-[var(--border-subtle)] px-4 py-3 text-xs leading-5 text-[var(--text-secondary)]">
        {label}. {candles.length} {copy.visible}; {copy.changed}:{" "}
        {change.toFixed(1)}%. {copy.overlays}{" "}
        <a
          href="https://www.tradingview.com/"
          target="_blank"
          rel="noopener noreferrer"
          className="font-semibold text-[var(--brand)] underline decoration-transparent underline-offset-2 hover:decoration-current"
        >
          {copy.powered} TradingView Lightweight Charts™
        </a>
        .
      </figcaption>
    </figure>
