...
const copy = {
  de: {
    ...
    weekly: "Wöchentliche Lernzeit in Minuten",
    motion: "Bewegung reduzieren",
    reminders: "E-Mail-Erinnerungen",
    saved: "Einstellungen gespeichert.",
    demo: "Sprache und Darstellung werden in diesem Browser gespeichert.",
  },
  sl: {
    language: "Jezik učenja",
    appearance: "Videz",
    weekly: "Tedenski čas učenja v minutah",
    motion: "Zmanjšaj gibanje",
    reminders: "E-poštni opomniki",
    saved: "Nastavitve so shranjene.",
    demo: "Jezik in videz se shranita v tem brskalniku.",
  },
  en: {
    language: "Learning language",
    appearance: "Appearance",
    weekly: "Weekly study minutes",
    motion: "Reduce motion",
    reminders: "Email reminders",
    saved: "Settings saved.",
    demo: "Language and appearance are saved in this browser.",
  },
};
...
export function SettingsPage() {
  ...
  const query = useQuery({
    queryKey: ["academy", "preferences"],
    queryFn: async () =>
      (await academyApi<PreferenceRead>("/preferences")) ?? null,
  ...
  const save = async () => {
    ...
    try {
      ...
      document.documentElement.dataset.reduceMotion = reducedMotion
        ? "true"
        : "false";
  ...
  return (
    <>
      <PageHeading
        ...
        title={dictionary.secondary.settingsTitle}
        description={dictionary.secondary.settingsIntro}
      />
      <section className="mx-auto max-w-3xl space-y-4">
        <Setting icon={Languages} title={strings.language}>
          <LanguageSwitcher />
        </Setting>
        <Setting icon={Palette} title={strings.appearance}>
          <ThemeSwitcher />
        </Setting>
        <Setting icon={Clock3} title={strings.weekly}>
          <input
            aria-label={strings.weekly}
            type="number"
            min={15}
            max={2400}
            step={15}
            ...
            className="numeric min-h-11 w-36 rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-[var(--surface-2)] px-3"
          />
        </Setting>
        <Setting icon={Bell} title={strings.motion}>
        ...
        <Setting icon={Bell} title={strings.reminders}>
          <label className="flex min-h-11 items-center gap-3">
            <input
              type="checkbox"
              checked={reminders}
        ...
        {mode === "demo" ? (
          <p className="text-sm text-[var(--text-secondary)]">{strings.demo}</p>
        ) : null}
        {status ? (
        ...
        <Button
          onClick={() => void save()}
          disabled={weekly < 15 || weekly > 2400}
        >
...
function Setting({
  ...
  return (
    <article className="flex flex-col gap-4 rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[var(--surface-1)] p-5 sm:flex-row sm:items-center sm:justify-between">
