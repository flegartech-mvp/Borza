...
export function AuthPage({ mode }: { mode: Mode }) {
  ...
  const submit = async (values: FormValues) => {
    ...
    if (
      (mode === "sign-in" ||
        mode === "register" ||
        mode === "update-password") &&
      !passwordSchema.safeParse(values.password).success
    ) {
      setError(dictionary.auth.error);
      return;
    }
  ...
  return (
    <main className="academy-grid grid min-h-dvh place-items-center px-3 py-8 sm:px-4 sm:py-10">
      <section className="w-full max-w-md overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border-strong)] bg-[var(--surface-1)] p-5 shadow-[var(--shadow-floating)] sm:p-8">
        ...
        {mode === "forgot" ? (
          <p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">
            {dictionary.auth.resetIntro}
          </p>
        ) : null}
        {!configured ? (
          <div className="mt-5 rounded-[var(--radius-sm)] border border-[var(--warning)] bg-[var(--warning-soft)] p-4">
            <p className="font-semibold">
              {dictionary.auth.configurationTitle}
            </p>
            <p className="mt-1 text-sm leading-6 text-[var(--text-secondary)]">
              {dictionary.auth.configurationBody}
            </p>
            <Link
              href="/home"
        ...
        {mode === "update-password" && !passwordRecovery ? (
          <p className="mt-5 rounded-[var(--radius-sm)] border border-[var(--warning)] bg-[var(--warning-soft)] p-4 text-sm">
            Open this page from the secure recovery link in your email.
          </p>
        ) : null}
        <form onSubmit={handleSubmit(submit)} className="mt-6 space-y-4">
          {mode === "register" ? (
            <label className="block text-sm font-medium">
              ...
              <input
                ...
                autoComplete="name"
                className="mt-2 w-full rounded-[var(--radius-sm)] border border-[var(--border-strong)] bg-[var(--surface-2)] px-3"
              />
              {errors.fullName ? (
                <span className="mt-1 block text-xs text-[var(--negative)]">
                  Required
                </span>
              ) : null}
            </label>
          ...
          <Button
            ...
            disabled={
              !configured || (mode === "update-password" && !passwordRecovery)
            }
            className="w-full"
          >
