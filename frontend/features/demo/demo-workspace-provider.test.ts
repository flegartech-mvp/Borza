...
describe("authenticated workspace contracts", () => {
  it("maps onboarding and sends the documented POST shape", async () => {
    ...
    expect(onboardingRequest(onboarding)).toMatchObject({
      weekly_study_minutes: 210,
      placement_score: 100,
    });
    expect(request).toHaveBeenCalledWith("/onboarding", {
      method: "POST",
      body: expect.objectContaining({
        learning_goal: onboarding.goal,
        answers: onboarding,
      }),
    });
  });
  ...
  it("uses PUT and DELETE contracts for progress, notes, and bookmarks", async () => {
    ...
    expect(request).toHaveBeenNthCalledWith(1, "/lessons/lesson-a/progress", {
      method: "PUT",
      body: {
        status: "completed",
    ...
    expect(request).toHaveBeenNthCalledWith(2, "/lessons/lesson-a/notes", {
      method: "PUT",
      body: { body: "My note" },
    });
    expect(request).toHaveBeenNthCalledWith(3, "/lessons/lesson-a/bookmark", {
      method: "PUT",
    });
    expect(request).toHaveBeenNthCalledWith(4, "/lessons/lesson-a/bookmark", {
      method: "DELETE",
    });
  });
  ...
  it("maps the journal schema and hydrates remote state without local demo data", async () => {
    ...
    expect(request).toHaveBeenCalledWith("/journal", {
      method: "POST",
      body: expect.objectContaining({
        market_context: journal.context,
        entry_price: 101,
        actual_risk: 50,
        rule_adherence: 100,
        lesson_learned: journal.lesson,
      }),
    });
    ...
    const dashboard: DashboardRead = {
      ...
      onboarding: {
        completed: true,
        recommended_path_id: "path-risk-management",
      },
      progress: [],
    ...
    expect(hydrated.state.onboarding?.recommendation).toBe(
