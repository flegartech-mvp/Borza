...
export const EMPTY_DEMO_STATE: DemoWorkspaceState = {
  version: 1,
  onboarding: null,
  completedLessons: [],
  bookmarks: [],
...
export function onboardingRequest(answers: OnboardingAnswers) {
  ...
  const correct = /position|positionsgröße|velikost pozicije/i.test(
    answers.placement,
  );
  return {
...
export const workspaceRequests = {
  saveOnboarding: (request: Requester, answers: OnboardingAnswers) =>
    request("/onboarding", {
      method: "POST",
      body: onboardingRequest(answers),
    }),
  completeLesson: (request: Requester, lessonId: string) =>
  ...
  setBookmark: (request: Requester, lessonId: string, bookmarked: boolean) =>
    request(`/lessons/${lessonId}/bookmark`, {
      method: bookmarked ? "PUT" : "DELETE",
    }),
  saveNote: (request: Requester, lessonId: string, note: string) =>
    request(`/lessons/${lessonId}/notes`, {
      method: "PUT",
      body: { body: note },
    }),
  addJournal: (request: Requester, entry: JournalEntry) =>
    request("/journal", { method: "POST", body: journalRequest(entry) }),
};
...
export function hydrateWorkspace(
  ...
  const onboarding: OnboardingAnswers | null = dashboard.onboarding.completed
    ? {
        goal: "",
        level: "",
        interest: "",
        ...
        placement: "",
        recommendation:
          dashboard.onboarding.recommended_path_id ??
  ...
  return {
    ...
    state: {
      ...
      quizScores: Object.fromEntries(
        progress
          .filter((item) => item.best_score !== null)
          .map((item) => [item.lesson_id, number(item.best_score)]),
      ),
      journalEntries: journal.map((item) => ({
        ...
        stop: number(item.stop_price),
        target: number(item.target_price),
        plannedRisk: number(item.planned_risk),
        actualRisk: number(item.actual_risk),
        resultAmount: number(item.result_amount),
        resultR: number(item.r_multiple),
        emotionBefore: item.emotions_before ?? "",
        emotionDuring: item.emotions_during ?? "",
        emotionAfter: item.emotions_after ?? "",
        followedRules: (item.rule_adherence ?? 0) >= 80,
        lesson: item.lesson_learned,
        chartSnapshotUrl: item.chart_snapshot_url ?? "",
        tags: item.tags,
      })),
      simulatorSummary: dashboard.simulator
        ? {
            completedAt: new Date().toISOString(),
            remoteSessionId: dashboard.simulator.session_id,
            netPnl: number(dashboard.simulator.realized_pnl),
            trades: 0,
            ruleViolations: 0,
            processScore: 0,
            winRate: 0,
...
export function DemoWorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const remote = useQuery({
    ...
    queryFn: fetchRemoteWorkspace,
    enabled: Boolean(user),
    retry: 1,
  });
  const state = user ? (remote.data?.state ?? EMPTY_DEMO_STATE) : localState;
  const request: Requester = useCallback(
    (path, options) => academyApi(path, options),
    [],
  );
  const invalidate = useCallback(async () => {
    await queryClient.invalidateQueries({ queryKey: REMOTE_QUERY_KEY });
  }, [queryClient]);
  const refresh = useCallback(async () => {
    if (user) await remote.refetch();
  }, [remote, user]);

  const saveOnboarding = useCallback(
    async (answers: OnboardingAnswers) => {
      if (user) {
        await workspaceRequests.saveOnboarding(request, answers);
        await invalidate();
  ...
  const completeLesson = useCallback(
    async (lessonId: string) => {
      if (user) {
        await workspaceRequests.completeLesson(request, lessonId);
        await invalidate();
        return;
      }
  ...
  const toggleBookmark = useCallback(
    async (lessonId: string) => {
      ...
      if (user) {
        await workspaceRequests.setBookmark(
          request,
          lessonId,
          !currentlyBookmarked,
        );
  ...
  const loadLessonNote = useCallback(
    async (lessonId: string) => {
      ...
      try {
        const note = await academyApi<{ body: string }>(
          `/lessons/${lessonId}/notes`,
        );
        queryClient.setQueryData<RemoteBundle>(REMOTE_QUERY_KEY, (current) =>
          current
            ? {
                ...current,
                state: {
  ...
  const saveLessonNote = useCallback(
    async (lessonId: string, note: string) => {
      ...
      writeState({
        ...state,
        lessonNotes: { ...state.lessonNotes, [lessonId]: normalized },
      });
    },
  ...
  const recordQuizScore = useCallback(
    async (
      ...
      if (user) {
        const result = await academyApi<QuizSubmissionResult>(
  ...
  const saveReviewCard = useCallback(
    async (
      ...
      if (user) {
        await academyApi(`/review/cards/${card.cardId}`, {
          method: "POST",
          body: {
            rating,
            due_at: card.due,
            ...
            state:
              ["new", "learning", "review", "relearning"][card.state] ?? "new",
            reps: card.reps,
            lapses: card.lapses,
      ...
      writeState({
        ...state,
        reviewCards: { ...state.reviewCards, [card.cardId]: card },
      });
    },
  ...
  const addJournalEntry = useCallback(
    async (entry: JournalEntry) => {
      if (user) {
        await workspaceRequests.addJournal(request, entry);
        await invalidate();
        return;
      }
      writeState({
  ...
  const saveSimulatorSummary = useCallback(
    async (summary: SimulatorSummary) => {
      if (user) {
        await invalidate();
        return;
      }
  ...
  const resetDemo = useCallback(() => {
    if (!user) writeState(EMPTY_DEMO_STATE);
  }, [user]);

  const value = useMemo<WorkspaceContextValue>(
    () => ({
      state,
      dashboard: user ? (remote.data?.dashboard ?? null) : null,
      mode: user ? "authenticated" : "demo",
      hydrating: Boolean(user && remote.isLoading),
      hydrationError: Boolean(user && remote.isError),
      refresh,
      loadLessonNote,
      saveOnboarding,
      completeLesson,
...
export function useDemoWorkspace(): WorkspaceContextValue {
  ...
  if (!context)
    throw new Error(
      "useDemoWorkspace must be used inside DemoWorkspaceProvider",
    );
  return context;
}
