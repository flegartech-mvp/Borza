...
export function resolveApiBaseUrl(
  ...
  if (value.startsWith("/") && !value.startsWith("//")) {
    if (/[?#\\]/.test(value)) {
      return invalid(
        "NEXT_PUBLIC_API_URL",
        "Use a same-origin path without query, fragment, or backslash.",
      );
    }
  ...
  try {
    ...
    if (
      ...
      return invalid(
        "NEXT_PUBLIC_API_URL",
        "Use a credential-free HTTP(S) URL without query or fragment.",
      );
    }
    return {
    ...
    return invalid(
      "NEXT_PUBLIC_API_URL",
      "Use an absolute HTTP(S) URL or same-origin path.",
    );
  }
}
...
export function getSupabasePublicConfig(): {
  ...
  if (!url || !publishableKey)
    return { configured: false, url, publishableKey };
  try {
