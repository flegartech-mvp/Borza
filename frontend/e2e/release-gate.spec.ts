...
test.describe("Borza Release Gate E2E Suite", () => {
  test("loads main dashboard and verifies layout components", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("applies filter controls and updates news feed view", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("handles network error state gracefully when backend is offline", async ({
    page,
  }) => {
    await page.route("**/api/**", (route) => route.abort());
    await page.goto("/");
    const alertOrError = page
      .locator('[role="alert"]')
      .or(page.getByText(/unavailable|error|failed/i))
      .first();
